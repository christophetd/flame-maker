...
public class WeightsModificationComponent extends JComponent {
	...
	public WeightsModificationComponent(final ObservableFlameBuilder flameBuilder) {
		...
		// 6 vertical groups, 2 horizontal
		...
		for(Variation variation: Variation.ALL_VARIATIONS) {
			...
			formattedTextField.addPropertyChangeListener("value", new PropertyChangeListener() {
				...
				public void propertyChange(PropertyChangeEvent evt) {
					double newWeight = ((Number) formattedTextField.getValue()).doubleValue();
					...
					if(concernedVariation != null) {
						flameBuilder.setVariationWeight(selectedTransformationIndex, concernedVariation, newWeight);
					}
				}
			});
	...
	private class WeightInputVerifier extends InputVerifier{
		...
		public boolean verify(JComponent input){
			...
			try {
				 ...
				 * Le champ est valide si le poids est compris entre 0 et 1
				 */
